...
export const Address = (dat) => async (dispatch,getState) => {
    try {
        // console.log("login",dat)
        const token = getState().normal.token;
        const { data } = await Axios.patch(`${port}/normal/address`, dat, { headers: { "x-auth-token": token }})
        window.location.reload()
        // dispatch({ type: NLOGIN_SUCCESS, payload: data })
    } catch (error) {
        // alert(error.response?.data?.msg)
...
export const getorder = ()=>async(dispatch) =>{
    try {
        // const token = localStorage.getItem('normaltoken')
        const { data } = await Axios.get(`${port}/pdt/order`)
...
export const delorder = (id)=>async(dispatch) =>{
    try {
        // const token = localStorage.getItem('normaltoken')
        console.log(id)
        const { data } = await Axios.delete(`${port}/pdt/order/${id}`)
        // console.log("data",data)
